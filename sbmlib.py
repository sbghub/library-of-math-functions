...
A circular prime is a number that is prime for every rotation of it's digits
...
'''
(7.) are the digits in each number the same jumbled
'''
...
def fibn(n):
    '''
    returns an error message if n is not a non-negative integer
    '''
    if type(n)!=int: return "type error"
